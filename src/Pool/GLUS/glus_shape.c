...
GLUSvoid GLUSAPIENTRY glusCreatePlanef(GLUSshape* shape, GLUSfloat halfExtend)
{
	GLUSuint i;
...
GLUSvoid GLUSAPIENTRY glusCreateCubef(GLUSshape* shape, GLUSfloat halfExtend)
{
	GLUSuint i;
...
GLUSvoid GLUSAPIENTRY glusCreateCubeOpenf(GLUSshape* shape, GLUSfloat halfExtend)
{
  GLUSuint i;

  GLUSuint numberVertices = 24;
  GLUSuint numberIndices = 36-6;
...
GLUSvoid GLUSAPIENTRY glusCreateSpheref(GLUSshape* shape, GLUSfloat radius, GLUSuint numberSlices)
{
	GLUSuint i,j;
...
GLUSvoid GLUSAPIENTRY glusCreatePlaneSlicedf(GLUSshape* shape, GLUSfloat halfExtend, GLUSuint numberSlices)
{
	GLUSuint i,j;
  ...
  float minX,maxX,minZ,maxZ, tx, tz;
  ...
  maxZ = +halfExtend;
	...
	for (i = 0; i < numberParallels + 1; i++ )
	{
      ...
      tx = ((float)i)/((float)numberParallels);
      tz = ((float)j)/((float)numberSlices);
			...
			shape->texCoords[texCoordsIndex + 0] = (GLUSfloat) j / (GLUSfloat) numberSlices;
			shape->texCoords[texCoordsIndex + 1] = ( (GLUSfloat)i ) / (GLUSfloat) (numberParallels - 1 );
		}
	}
...
GLUSAPI GLUSvoid GLUSAPIENTRY glusCreateTorusf(GLUSshape* shape, GLUSfloat innerRadius, GLUSfloat outerRadius, GLUSuint numSides, GLUSuint numFaces)
{
	...
	for (sideCount = 0; sideCount <= numSides; ++sideCount, s+=sIncr)
	{
		...
		for (faceCount = 0; faceCount <= numFaces; ++faceCount, t+=tIncr)
		{
			...
			shape->vertices [indexVertices + 0] = (outerRadius + innerRadius * cos2PIt) * cos2PIs;
			shape->vertices [indexVertices + 1] = (outerRadius + innerRadius * cos2PIt) * sin2PIs;
			shape->vertices [indexVertices + 2] = innerRadius * sin2PIt;		
